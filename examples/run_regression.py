"""
Regression benchmark — all available frameworks × all 27 datasets.
...
-----
    ...
    # custom options
    python3 run_regression.py --n_runs 5 --max_samples 50000
    python3 run_regression.py --n_runs 1 --max_samples 10000 --datasets abalone kin8nm
    python3 run_regression.py --n_runs 10 --max_samples 100000 --output_dir my_results
    python3 run_regression.py --n_runs 10 --parallel --cpu_utilization 0.8
...
---------
    ...
    --datasets      Space-separated list of dataset names.  Default: all 27.
...
def parse_args():
    p = argparse.ArgumentParser(description="SAMLB regression benchmark")
    ...
    p.add_argument("--datasets",    nargs="+",  default=None, help="dataset names (default: all 27)")
...
def main():
    ...
    models = _build_models(seed=seeds[0] if seeds else 42)

    print("SAMLB Regression Benchmark", flush=True)
    print(f"  n_runs:      {args.n_runs}", flush=True)
    print(
        f"  max_samples: {args.max_samples if args.max_samples else 'no cap (full dataset)'}",
        flush=True,
    )
    print(f"  datasets:    {'all 27' if args.datasets is None else args.datasets}", flush=True)
    print(f"  output_dir:  {args.output_dir}", flush=True)
    print(f"  seeds:       {seeds}", flush=True)
    print(f"  frameworks:  {list(models)}", flush=True)
    print(f"  parallel:    {args.parallel}", flush=True)
    if "ChaCha" not in models:
        print(
            "  optional:    ChaCha unavailable (install with `pip install \"flaml[vw]\"`)",
            flush=True,
        )
    if args.parallel:
            ...
            with ProcessPoolExecutor(max_workers=workers) as ex:
                ...
                while pending:
                    ...
                    for fut in done:
                        ...
                        if ok:
                            avg_r2 = sum(r.primary_metric() for r in ok) / len(ok)
                            print(
    ...
    suite.to_csv(f"{args.output_dir}/regression_{args.n_runs}runs{cap_tag}.csv")
