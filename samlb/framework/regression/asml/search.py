"""
ASML Regression — Pipeline search with ARDNS strategy.

Mirrors the classification variant but with regression models and
a configurable feature-selection toggle.
"""
...
class PipelineSearch:
    """
    Creates and manages River regression pipelines for ASML.
    ...
    feature_selection : bool
        Whether to include a feature-selection step.
    budget : int
        Total pipelines to maintain.
    seed : int, optional
        Random seed.
    """
    ...
    def _random_pipeline(self):
        ...
        if self.use_fs and self.feature_selection_methods and random.random() > 0.5:
            fs = random.choice(self.feature_selection_methods)
            return (pre | fs | algo).clone()
        return (pre | algo).clone()
    ...
    def select_and_update_pipelines(self, best_pipeline) -> list:
        ...
        randoms = [self._random_pipeline() for _ in range(self.random_budget)]
